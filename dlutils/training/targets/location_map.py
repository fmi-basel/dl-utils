...
def generate_locationmap(
    ...
    if isinstance(period, (list, tuple)):
        if len(period) == 1:
            ...
            raise ValueError(
                'wrong dimension of period argument, expected 1 or {}, got: {}'.format(
                    ndim, len(period)))
    ...
    if isinstance(offset, (list, tuple)):
        if len(offset) == 1:
            ...
            raise ValueError(
                'wrong dimension of offset argument, expected 1 or {}, got: {}'.format(
                    ndim, len(offset)))
    ...
    def sine_parametric_f(amplitude, period, offset):
        ...
        def f(*args):
            return sum(f1d(*params)
                       for params in zip(args, amplitude, w, phase))
        return f
    ...
    def triangle_parametric_f(amplitude, period, offset):
        ...
        def f(*args):
            return sum(f1d(*params)
                       for params in zip(args, amplitude, period, offset))
        return f
    ...
    for n in range(1, ndim + 1):
        ...
        loc_map[..., n] = np.fromfunction(parametric_f(
            amplitude, period, offset + relative_offset), shape, dtype=np.float32)
...
def generate_locationmap_target(segmentation, location_map):
    ...
    location_map_mean = np.zeros_like(
        location_map, dtype=np.float32)  # -2. # background --> label= -2
