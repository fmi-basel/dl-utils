...
def cosine_embedding_loss(neighbor_distance=7, include_background=False):#TODO
    ...
    def generate_instance_mask(segmentation, label):
        ...
        neighbour_labels = np.unique(segmentation*neighborhood.astype(np.int))
    ...
    def while_body(segmentation, embeddings, labels, loss, i):
        ...
        center_weights, surround_weights = tf.py_func(generate_instance_mask, [segmentation, labels[i]], [tf.float32, tf.float32])
        center_weights.set_shape( segmentation.get_shape() )
        surround_weights.set_shape( segmentation.get_shape() )
        
        center_mean = tf.reduce_sum(embeddings*center_weights, axis=list(range(K.ndim(embeddings)-1)), keep_dims=True)
        center_mean = tf.nn.l2_normalize(center_mean, dim=-1)
        
        cos_similarity = tf.reduce_sum(embeddings * center_mean, axis=-1, keep_dims=True)
    ...
    def unbatched_loss(packed_inputs):
        ...
        loss = tf.while_loop(while_condition, while_body, [segmentation, embeddings, labels, loss, i])[-2]
    ...
    def loss(y_true, y_pred):
        ...
        dilation_diameter = np.broadcast_to(dilation_diameter, K.ndim(y_true)-2)
